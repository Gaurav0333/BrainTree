...
/**
 * Get saved GooglePay customer payment method instrument
 * @return {dw.util.Collection} payment instruments
 */
customerHelper.getGooglePayCustomerPaymentInstrument = function () {
    var customerPaymentInstruments = customerHelper.getCustomerPaymentInstruments(prefs.paymentMethods.BRAINTREE_GOOGLEPAY.paymentMethodId);
    if (customerPaymentInstruments) {
        var iterator = customerPaymentInstruments.iterator();
        var paymentInst = null;
        while (iterator.hasNext()) {
            paymentInst = iterator.next();
            if (!empty(paymentInst.creditCardToken)) {
                return paymentInst;
            }
        }
    }
